...
└─►│ │ memory  │ ├───►│        │ memory │        ├───┐
   │ │Password │ │    │        │Password│        │   │
   │ └─────────┘ │    │        └────────┘        │   │
   │  Argon2     │    │  Encrypted Zip Created   │   │
   │  H A S H    │    │   with both files inside │   │
   └─────────────┘    └──────────────────────────┘   │
                                                     │
  ┌─────────────────────────────────────────────┐    │
  │                                             │    │
  │   Argon2  Hash  is  reprinted  on  screen   │◄───┘
  │                                             │
  └─────────────────────────────────────────────┘
```
...
Sometimes the hashed password doesnt work out of the box. We need to modify it to be accepted into the password rules for the service or website we're trying to login to. If the hashed password, for some reason, doesnt meat the requirements on the website you're trying to use it with -- here are my suggestions:
